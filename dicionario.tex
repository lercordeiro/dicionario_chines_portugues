...
\DeclareDocumentEnvironment{verbete}{ o m }{%
  \leavevmode%
  \IfValueTF{#1}%
...
\DeclareDocumentCommand{\significado}{ o m m }{%
  \vspace{.8ex}$\hookrightarrow$\ (\textit{#2})%
  \IfValueT{#1}{\small [P.C.:\xpinyin*{#1}]}%
  \vspace{.5ex}
  {\small \mylist{#3}}%
}
...
\titlespacing{\section}{12pc}{.5ex plus .1ex minus .2ex}{0pc}
