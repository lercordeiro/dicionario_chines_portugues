...
\xpinyinsetup{ratio={.8},vsep={1.2em},multiple={\color{Sepia}}}
...
%\begin{enumerate*}[mode=boxed,itemjoin={{; }},itemjoin*={{; e }}]
\begin{enumerate*}[mode=boxed,itemjoin={{; }}]
\ProcessList{#1}{ \insertitem }
\end{enumerate*}\\
}
\newcommand\insertitem[1]{\item #1}

\newcounter{pronuncias}

\DeclareDocumentEnvironment{verbete}{ o m o }{%
  \leavevmode%
  \IfValueTF{#1}%
    {\markboth{#2«\pinyin{#1}»}{#2«\pinyin{#1}»}\index{#2«\pinyin{#1}»}}%
    {\markboth{#2}{#2}\index{#2}}%
  \setcounter{pronuncias}{0}
  \begin{minipage}[t][][t]{\linewidth}%
  \RaggedRight%
  {\Large \textbf{#2}%
   \IfValueT{#3}%
     {{\hspace{.5ex}\textsuperscript{\tiny(#3画)}}}%
  }%
}{%
  \end{minipage}
}

\DeclareDocumentEnvironment{pronuncia}{ o m o }{%
  \stepcounter{pronuncias}
  \IfValueT{#1}{#1}%
  \ifthenelse{\value{pronuncias} > 1}%
    {$\bullet$ «\pinyin{#2}»}%
    {«\pinyin{#2}»}%
  \IfValueT{#3}{ #3}\\%
}{}

\DeclareDocumentCommand{\significado}{ s o m m }{%
  \vspace{.8ex}%
  \IfBooleanTF{#1}
    {$\hookrightarrow$\ Veja: #4\\}
    {$\hookrightarrow$\ (\textit{#3})%
       \IfValueT{#2}{{\tiny\ [P.C.:\xpinyin*{#2}]}}%
     {\small \mylist{#4}}
    }
}
...
\titlespacing{\section}{1pc}{.5ex plus .1ex minus .2ex}{0pc}
...
%\input{verbetes/I.tex} %%%%% Não existem palavras com pinyin iniciado em "I"
...
%\input{verbetes/U.tex} %%%%% Não existem palavras com pinyin iniciado em "U"
%\input{verbetes/V.tex} %%%%% Não existem palavras com pinyin iniciado em "V"
