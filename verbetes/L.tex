%%%
%%% L
%%%
\section*{L}
\addcontentsline{toc}{section}{L}
\begin{multicols*}{2}

\begin{verbete}[拉拉队]{la1la1dui4}
\significado{la1la1dui4}{n.}{
    claque; torcida
}
\end{verbete}

\begin{verbete}[辣]{la4}
\significado{la4}{adj.}{
    picante
}
\end{verbete}

\begin{verbete}[来]{lai2}
\significado{lai2}{v.}{
    vir; trazer
}
\end{verbete}

\begin{verbete}[蓝]{lan2}
\significado{lan2}{adj.}{
    azul
}
\end{verbete}

\begin{verbete}[蓝色]{lan2se4}
\significado{lan2se4}{n.}{
    cor azul
}
\end{verbete}

\begin{verbete}[篮球]{lan2qiu2}
\significado{lan2qiu2}{n.}{
    basquetebol
}
\end{verbete}

\begin{verbete}[老板]{lao3ban3}
\significado{lao3ban3}{n.}{
    patrão, patroa
}
\end{verbete}

\begin{verbete}[老家]{lao3jia1}
\significado{lao3jia1}{n.}{
    terra natal
}
\end{verbete}

\begin{verbete}[老人家]{lao3ren2jia0}
\significado{lao3ren2jia0}{n.}{
    senhor ancião; madame; senhora
}
\end{verbete}

\begin{verbete}[老师]{lao3shi1}
\significado{lao3shi1}{n.}{
    professor, professora
}
\end{verbete}

\begin{verbete}[了]{le0}
\significado{le0}{part.}{
    partícula para denotar mudança
}
\end{verbete}

\begin{verbete}[冷]{leng3}
\significado{leng3}{adj.}{
    frio, fria
}
\end{verbete}

\begin{verbete}[累]{lei4}
\significado{lei4}{adj.}{
    cansado
}
\end{verbete}

\begin{verbete}[离]{li2}
\significado{li2}{n.}{
    (ser longe) de ... até...
}
\end{verbete}

\begin{verbete}[里]{li3}
\significado{li3}{p.l.}{
    em; dentro; interior
}
\end{verbete}

\begin{verbete}[里斯本]{Li3si1ben3}
\significado{Li3si1ben3}{n.}{
    Lisboa
}
\end{verbete}

\begin{verbete}[里斯本大学]{Li3si1ben3\ Da4xue2}
\significado{Li3si1ben3\ Da4xue2}{n.}{
    Universidade de Lisboa
}
\end{verbete}

\begin{verbete}[礼节]{li3jie2}
\significado{li3jie2}{n.}{
    cortesia; protocolo; cerimônia
}
\end{verbete}

\begin{verbete}[礼物]{li3wu4}
\significado{li3wu4}{n.}{
    prenda; lembrança; presente|
    \pc{件}
}
\end{verbete}

\begin{verbete}[厉害]{li4hai0}
\significado{li4hai0}{adj.}{
    severo; rigoroso; exigente
}
\end{verbete}

\begin{verbete}[历史]{li4shi3}
\significado{li4shi3}{n.}{
    história
}
\end{verbete}

\begin{verbete}[脸]{lian3}
\significado{lian3}{n.}{
    cara; rosto; face
}
\end{verbete}

\begin{verbete}[练习]{lian4xi2}
\significado{lian4xi2}{n.}{
    exercício
}
\significado{lian4xi2}{v.}{
    praticar; exercitar
}
\end{verbete}

\begin{verbete}[凉快]{liang2kuai0}
\significado{liang2kuai0}{adj.}{
    agradável; fresco, fresca
}
\end{verbete}

\begin{verbete}[两]{liang3}
\significado{liang3}{num.}{
    2|
    dois, duas|
    (sempre usado antes de p.c.)
}
\end{verbete}

\begin{verbete}[辆]{liang4}
\significado{liang4}{p.c.}{
    palavra classificadora para automóveis
}
\end{verbete}

\begin{verbete}[邻居]{lin2ju1}
\significado{lin2ju1}{n.}{
    vizinho
}
\end{verbete}

\begin{verbete}[零/\Circle]{ling2}
\significado{ling2}{num.}{
    0|
    zero
}
\end{verbete}

\begin{verbete}[领导]{ling3dao3}
\significado{ling3dao3}{n.}{
    chefe; dirigente
}
\end{verbete}

\begin{verbete}[流利]{liu2li4}
\significado{liu2li4}{adj.}{
    fluente
}
\end{verbete}

\begin{verbete}[六]{liu4}
\significado{liu4}{num.}{
    6|
    seis
}
\end{verbete}

\begin{verbete}[遛狗]{liu4gou3}
\significado{liu4gou3}{v.+compl.}{
    passear o cão
}
\end{verbete}

\begin{verbete}[伦敦]{lun2dun1}
\significado{lun2dun1}{n.}{
    Londres
}
\end{verbete}

\begin{verbete}[龙]{long2}
\significado{long2}{n.}{
    dragão
}
\end{verbete}

\begin{verbete}[龙山]{Long2shan1}
\significado{Long2shan1}{n.}{
    Longshan
}
\end{verbete}

\begin{verbete}[楼]{lou2}
\significado{lou2}{n.}{
    edifício; prédio
}
\end{verbete}

\begin{verbete}[录音]{lu4yin1}
\significado{lu4yin1}{v.+compl.}{
    gravar
}
\end{verbete}
\significado{lu4yin1}{n.}{
    gravação
}
\end{verbete}

\begin{verbete}[路]{lu4}
\significado{lu4}{n.}{
    caminho; via
}
\end{verbete}

\begin{verbete}[路口]{lu4kou3}
\significado{lu4kou3}{n.}{
    cruzamento; interseção
}
\end{verbete}

\begin{verbete}[录像带]{lu4xiang4dai4}
\significado{lu4xiang4dai4}{n.}{
    video-cassete|
    \pc{盘}
}
\end{verbete}

\begin{verbete}[录像机]{lu4xiang4ji1}
\significado{lu4xiang4ji1}{n.}{
    gravador de vídeo|
    \pc{台}
}
\end{verbete}

\begin{verbete}[录音机]{lu4yin1ji1}
\significado{lu4yin1ji1}{n.}{
    gravador|
    \pc{台}
}
\end{verbete}

\begin{verbete}[旅行]{lv3xing2}
\significado{lv3xing2}{v.}{
    viajar
}
\end{verbete}

\begin{verbete}[旅游]{lv3you2}
\significado{lv3you2}{v.}{
    viajar
}
\end{verbete}

\begin{verbete}[绿]{lv4}
\significado{lv4}{adj.}{
    verde
}
\end{verbete}

\begin{verbete}[绿色]{lv4se4}
\significado{lv4se4}{n.}{lv4se4}{
    cor verde
}
\end{verbete}

\end{multicols*}
