%%%
%%% A
%%%

\section*{A}\addcontentsline{toc}{section}{A}

\begin{entry}{阿}{a1}{7}[Radical ⾩]
  \definition{pref.}{utilizado para indicar familiaridade antes de nomes monossilábicos, termos de parentesco, etc.}
  \seeref{阿}{e1}
\end{entry}

\begin{entry}{阿哥}{a1ge1}{7,10}[Radicais ⾩、⼝]
  \definition{s.}{irmão mais velho (familiar)}
\end{entry}

\begin{entry}{阿姨}{a1yi2}{7,9}[HSK 4][Radicais ⾩、⼥]
  \definition[个,位]{s.}{tia; uma forma de tratamento para uma mulher da geração dos pais; dirigir-se a uma mulher que tem aproximadamente a mesma idade da sua mãe, geralmente não é parente | babá em uma família; professora em um jardim de infância | tia; irmã da mãe (mais comum no sul da China)}
\end{entry}

\begin{entry}{呵}{a1}{8}[Radical ⼝]
  \variantof{啊}
  \seeref{呵}{he1}
\end{entry}

\begin{entry}{啊}{a1}{10}[HSK 2][Radical ⼝]
  \definition{interj.}{Ah! | Oh! | interjeição de surpresa}
  \seeref{啊}{a2}
  \seeref{啊}{a3}
  \seeref{啊}{a4}
  \seeref{啊}{a5}
\end{entry}

\begin{entry}{啊呀}{a1ya1}{10,7}[Radicais ⼝、⼝]
  \definition{interj.}{Oh meu Deus! | interjeição de surpresa}
\end{entry}

\begin{entry}{啊哟}{a1yo5}{10,9}[Radicais ⼝、⼝]
  \definition{interj.}{Meu Deus! | Oh! | Ai! | interjeição de surpresa ou dor}
\end{entry}

\begin{entry}{啊}{a2}{10}[HSK 2][Radical ⼝]
  \definition{interj.}{Eh? | Que? | interjeição expressando dúvida ou exigindo resposta}
  \seeref{啊}{a1}
  \seeref{啊}{a3}
  \seeref{啊}{a4}
  \seeref{啊}{a5}
\end{entry}

\begin{entry}{嗄}{a2}{13}[Radical ⼝]
  \definition{adj.}{rouco}
  \variantof{啊}
\end{entry}

\begin{entry}{啊}{a3}{10}[HSK 2][Radical ⼝]
  \definition{interj.}{Eh? | Meu! | E aí? | Que? | interjeição de surpresa ou dúvida}
  \seeref{啊}{a1}
  \seeref{啊}{a2}
  \seeref{啊}{a4}
  \seeref{啊}{a5}
\end{entry}

\begin{entry}{啊}{a4}{10}[HSK 2][Radical ⼝]
  \definition{interj.}{Ah! | OK! | Oh, é você! | Hum! | expressão de reconhecimento | interjeição de acordo}
  \seeref{啊}{a1}
  \seeref{啊}{a2}
  \seeref{啊}{a3}
  \seeref{啊}{a5}
\end{entry}

\begin{entry}{啊}{a5}{10}[HSK 2,4][Radical ⼝]
  \definition{adv.}{assim por diante}
  \definition{part.}{no final de sentença para expressar admiração | no final de sentença mostrando afirmação, aprovação, urgência, aconselhamento, etc. | no final de sentença para indicar uma pergunta | para pausar ligeiramente uma frase, chamando a atenção para as palavras seguintes | após cada um dos itens listados}
  \seeref{啊}{a1}
  \seeref{啊}{a2}
  \seeref{啊}{a3}
  \seeref{啊}{a4}
\end{entry}

\begin{entry}{矮}{ai3}{13}[HSK 4][Radical ⽮]
  \definition{adj.}{baixo em estatura, dimensão, grau ou ranque | curto (em comprimento)}
\end{entry}

\begin{entry}{矮凳}{ai3deng4}{13,14}[Radicais ⽮、⼏]
  \definition{s.}{banquinho baixo | banqueta}
\end{entry}

\begin{entry}{矮林}{ai3lin2}{13,8}[Radicais ⽮、⽊]
  \definition{s.}{mato | mata}
\end{entry}

\begin{entry}{矮胖}{ai3pang4}{13,9}[Radicais ⽮、⾁]
  \definition{adj.}{atarracado |  gorducho | rechonchudo | roliço | curto e robusto}
\end{entry}

\begin{entry}{矮人}{ai3ren2}{13,2}[Radicais ⽮、⼈]
  \definition{s.}{anão | homúnculo | nanismo}
\end{entry}

\begin{entry}{矮树}{ai3shu4}{13,9}[Radicais ⽮、⽊]
  \definition{s.}{arbusto | árvore pequena}
\end{entry}

\begin{entry}{矮小}{ai3 xiao3}{13,3}[HSK 4][Radicais ⽮、⼩]
  \definition{adj.}{subdimensionado; curto e pequeno; baixo e pequeno}
\end{entry}

\begin{entry}{矮星}{ai3xing1}{13,9}[Radicais ⽮、⽇]
  \definition{s.}{estrela anã}
\end{entry}

\begin{entry}{矮子}{ai3zi5}{13,3}[Radicais ⽮、⼦]
  \definition{s.}{pessoa baixa | anão}
\end{entry}

\begin{entry}{爱}{ai4}{10}[HSK 1][Radical ⽖]
  \definition*{s.}{sobrenome Ai}
  \definition[个]{s.}{amor; afeição; afeição profunda; preocupação profunda; especialmente amor entre pessoas}
  \definition{v.}{amar; ter sentimentos profundos por pessoas ou coisas | gostar; gostar de; estar interessado em |  cuidar; valorizar; ter em alta estima; cuidar bem de | estar apto a; ter o hábito de}
\end{entry}

\begin{entry}{爱爱}{ai4'ai5}{10,10}[Radicais ⽖、⽖]
  \definition{v.}{(coloquial) fazer amor}
\end{entry}

\begin{entry}{爱抚}{ai4fu3}{10,7}[Radicais ⽖、⼿]
  \definition{s.}{cuidado afetuoso | carinho}
  \definition{v.}{acariciar | cuidar (com ternura)}
\end{entry}

\begin{entry}{爱国}{ai4 guo2}{10,8}[HSK 4][Radicais ⽖、⼞]
  \definition{adj.}{patriótico}
  \definition{v.}{ser patriota; amar o seu país}
\end{entry}

\begin{entry}{爱好}{ai4 hao4}{10,6}[HSK 1][Radicais ⽖、⼥]
  \definition[个,种]{s.}{passatempo; interesse; \emph{hobby}; sentimentos de interesse especial ou afeição por algo}
  \definition{v.}{estar interessado em; ter prazer em; ter um forte interesse em algo; ter sentimentos profundos por alguém ou algo}
\end{entry}

\begin{entry}{爱好者}{ai4 hao4 zhe3}{10,6,8}[Radicais ⽖、⼥、⽼]
  \definition{s.}{amador | entusiasta | fã | amante de arte, esportes, etc.}
\end{entry}

\begin{entry}{爱护}{ai4hu4}{10,7}[HSK 4][Radicais ⽖、⼿]
  \definition{v.}{acalentar; valorizar; salvaguardar; cuidar bem de}
\end{entry}

\begin{entry}{爱情}{ai4qing2}{10,11}[HSK 2][Radicais ⽖、⼼]
  \definition{s.}{amor (entre pessoas) | afeição}
\end{entry}

\begin{entry}{爱人}{ai4ren5}{10,2}[HSK 2][Radicais ⽖、⼈]
  \definition[个]{s.}{esposa | amor, amada}
\end{entry}

\begin{entry}{爱上}{ai4shang4}{10,3}[Radicais ⽖、⼀]
  \definition{v.}{apaixonar-se por | ser gentil com}
\end{entry}

\begin{entry}{爱心}{ai4xin1}{10,4}[HSK 3][Radicais ⽖、⼼]
  \definition[片]{s.}{amor | cuidado | compaixão}
\end{entry}

\begin{entry}{碍事}{ai4shi4}{13,8}[Radicais ⽯、⼅]
  \definition{s.}{(usualmente em frases negativas) sem consequência, não importa}
  \definition{v.+compl.}{estar no caminho | ser um obstáculo}
\end{entry}

\begin{entry}{安}{an1}{6}[HSK 4][Radical ⼧]
  \definition{adj.}{pacífico; quieto; tranquilo; calmo; estáve; sem perturbação | seguro; protegido; com boa saúde; em paz; bem}
  \definition{pron.}{onde; como}
  \definition{s.}{segurança; proteção; paz; conforto | ampère; (eletricidade) abreviação de ampère}
  \definition{v.}{deixar (a mente de alguém) à vontade; acalmar; estabilizar | satisfazer; estar satisfeito; sentir-se satisfeito e à vontade | colocar em uma posição adequada; encontrar um lugar para | instalar; consertar; encaixar; configurar | trazer (uma acusação contra alguém); dar (a alguém um apelido) | abrigar (uma intenção); manter; segurar}
\end{entry}

\begin{entry}{安家}{an1jia1}{6,10}[Radicais ⼧、⼧]
  \definition{v.+compl.}{montar uma casa | estabelecer-se}
\end{entry}

\begin{entry}{安静}{an1jing4}{6,14}[HSK 2][Radicais ⼧、⾭]
  \definition{adj.}{quieto | tranquilo | pacífico | calmo}
\end{entry}

\begin{entry}{安排}{an1pai2}{6,11}[HSK 3][Radicais ⼧、⼿]
  \definition{s.}{arranjos | planos}
  \definition{v.}{organizar | programar | fazer planos}
\end{entry}

\begin{entry}{安全}{an1quan2}{6,6}[HSK 2][Radicais ⼧、⼊]
  \definition{adj.}{seguro}
  \definition{s.}{segurança}
\end{entry}

\begin{entry}{安神}{an1shen2}{6,9}[Radicais ⼧、⽰]
  \definition{v.+compl.}{acalmar os nervos | aliviar a inquietação pela tranquilização da mente e do corpo}
\end{entry}

\begin{entry}{安置}{an1zhi4}{6,13}[HSK 4][Radicais ⼧、⽹]
  \definition{v.}{providenciar; encontrar um lugar para; ajudar a estabelecer-se; colocar pessoas ou coisas em uma determinada posição ou organizá-las adequadamente}
\end{entry}

\begin{entry}{安装}{an1zhuang1}{6,12}[HSK 3][Radicais ⼧、⾐]
  \definition{v.}{instalar | consertar | configurar}
\end{entry}

\begin{entry}{按}{an4}{9}[HSK 3][Radical ⼿]
  \definition{v.}{pressionar | empurrar para baixo | deixar de lado | arquivar | restringir | controlar}
\end{entry}

\begin{entry}{按时}{an4shi2}{9,7}[HSK 4][Radicais ⼿、⽇]
  \definition{adv.}{na hora; no horário; pontualmente; de acordo com o tempo estipulado}
\end{entry}

\begin{entry}{按照}{an4zhao4}{9,13}[HSK 3][Radicais ⼿、⽕]
  \definition{prep.}{de acordo com | em conformidade com | à luz de | com base em}
\end{entry}

\begin{entry}{暗}{an4}{13}[HSK 4][Radical ⽇]
  \definition{adj.}{escuro; opaco; sem graça; pouca luz | escondido; secreto; não revelado | pouco claro; nebuloso; vago; confuso | subterrâneo}
  \definition{adv.}{secretamente | no escuro}
\end{entry}

\begin{entry}{暗恋}{an4lian4}{13,10}[Radicais ⽇、⼼]
  \definition{s.}{amor secreto}
  \definition{v.}{estar secretamente apaixonado por}
\end{entry}

\begin{entry}{暗示}{an4shi4}{13,5}[HSK 4][Radicais ⽇、⽰]
  \definition[个]{s.}{sugestão; insinuação; intimação; (psicologia) refere-se ao uso de palavras, gestos, expressões, etc. para fazer as pessoas aceitarem involuntariamente uma determinada opinião ou fazerem algo}
  \definition{v.}{dar uma dica; sugerir secretamente; indicar algo a alguém usando outras palavras, expressões faciais ou gestos sem dizer em voz alta}
\end{entry}

\begin{entry}{暗香}{an4xiang1}{13,9}[Radicais ⽇、⾹]
  \definition{s.}{fragrância sutil}
\end{entry}

\begin{entry}{奥}{ao4}{12}[Radical ⼤]
  \definition{adj.}{obscuro | misterioso}
\end{entry}

\begin{entry}{奥林匹克运动会}{ao4lin2pi3ke4 yun4dong4hui4}{12,8,4,7,7,6,6}[Radicais ⼤、⽊、⼖、⼗、⾡、⼒、⼈]
  \definition*{s.}{Jogos Olímpicos, Olimpíadas}
\end{entry}

\begin{entry}{奥特曼}{ao4te4man4}{12,10,11}[Radicais ⼤、⽜、⽈]
  \definition*{s.}{\emph{Ultraman},  super-herói de ficção científica japonesa}
\end{entry}

\begin{entry}{奥运}{ao4yun4}{12,7}[Radicais ⼤、⾡]
  \definition*{s.}{Jogos Olímpicos, Olimpíadas, abreviação de 奥林匹克运动会}
  \seealsoref{奥林匹克运动会}{ao4lin2pi3ke4 yun4dong4hui4}
\end{entry}

\begin{entry}{奥运会}{ao4yun4hui4}{12,7,6}[Radicais ⼤、⾡、⼈]
  \definition*{s.}{Jogos Olímpicos, Olimpíadas, abreviação de 奥林匹克运动会}
  \seealsoref{奥林匹克运动会}{ao4lin2pi3ke4 yun4dong4hui4}
\end{entry}

\begin{entry}{澳}{ao4}{15}[Radical ⽔]
  \definition*{s.}{Austrália, abreviação de 澳大利亚}
  \seealsoref{澳大利亚}{ao4da4li4ya4}
\end{entry}

\begin{entry}{澳大利亚}{ao4da4li4ya4}{15,3,7,6}[Radicais ⽔、⼤、⼑、⼆]
  \definition*{s.}{Austrália}
\end{entry}

%%%%% EOF %%%%%

