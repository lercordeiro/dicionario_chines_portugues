%%%
%%% F
%%%

\section*{F}\addcontentsline{toc}{section}{F}

\begin{entry}{发}{fa1}{5}[HSK 2][Radical ⼜]
  \definition{clas.}{para tiros (rodadas)}
  \definition{v.}{enviar | mandar}
  \seeref{发}{fa4}
\end{entry}

\begin{entry}{发表}{fa1biao3}{5,8}[HSK 3][Radicais ⼜、⾐]
  \definition{v.}{publicar; entregar; emitir; expressar; anunciar | publicar}
\end{entry}

\begin{entry}{发财}{fa1cai2}{5,7}[Radicais ⼜、⾙]
  \definition{v.+compl.}{ficar rico | fazer fortuna}
\end{entry}

\begin{entry}{发愁}{fa1chou2}{5,13}[Radicais ⼜、⼼]
  \definition{v.+compl.}{preocupar-se | ficar ansioso | ficar triste}
\end{entry}

\begin{entry}{发出}{fa1 chu1}{5,5}[HSK 3][Radicais ⼜、⼐]
  \definition{v.}{fazer; produzir; deixar sair | emitir; anunciar | enviar; partir | dar; emitir}
\end{entry}

\begin{entry}{发达}{fa1da2}{5,6}[HSK 3][Radicais ⼜、⾡]
  \definition{adj.}{desenvolvido; florescente}
  \definition{v.}{desenvolver; promover; florescer}
\end{entry}

\begin{entry}{发动}{fa1dong4}{5,6}[HSK 3][Radicais ⼜、⼒]
  \definition{v.}{iniciar; lançar; ligar motor; dar a partida (motor de combustão interna) | chamar à ação; mobilizar; estimular; despertar}
\end{entry}

\begin{entry}{发动机}{fa1dong4ji1}{5,6,6}[Radicais ⼜、⼒、⽊]
  \definition[台]{s.}{motor}
\end{entry}

\begin{entry}{发抖}{fa1dou3}{5,7}[Radicais ⼜、⼿]
  \definition{v.}{tremer | sacudir | estremecer}
\end{entry}

\begin{entry}{发挥}{fa1hui1}{5,9}[HSK 4][Radicais ⼜、⼿]
  \definition{v.}{colocar em jogo; dar jogo a; dar espaço a; dar rédea solta a; revelar a natureza ou a capacidade interior | expressar; desenvolver (uma ideia, um tema, etc.); elaborar; fazer valer o ponto ou o motivo}
\end{entry}

\begin{entry}{发明}{fa1ming2}{5,8}[HSK 3][Radicais ⼜、⽇]
  \definition[个]{s.}{invenção}
  \definition{v.}{inventar | expor; explicar}
\end{entry}

\begin{entry}{发明者}{fa1ming2zhe3}{5,8,8}[Radicais ⼜、⽇、⽼]
  \definition{s.}{inventor}
\end{entry}

\begin{entry}{发票}{fa1piao4}{5,11}[HSK 4][Radicais ⼜、⽰]
  \definition[张]{s.}{conta; recibo; fatura; recibos emitidos por lojas ou outros escritórios de cobrança}
\end{entry}

\begin{entry}{发烧}{fa1shao1}{5,10}[HSK 4][Radicais ⼜、⽕]
  \definition{v.}{ter febre; a temperatura corporal normal de uma pessoa é de cerca de 37ºC; se exceder 37,5ºC, é febre}
\end{entry}

\begin{entry}{发生}{fa1sheng1}{5,5}[HSK 3][Radicais ⼜、⽣]
  \definition{v.}{ocorrer; acontecer; tomar lugar}
\end{entry}

\begin{entry}{发送}{fa1 song4}{5,9}[HSK 3][Radicais ⼜、⾡]
  \definition{v.}{enviar; despachar | transmitir; enviar}
\end{entry}

\begin{entry}{发现}{fa1xian4}{5,8}[HSK 2][Radicais ⼜、⾒]
  \definition{s.}{descoberta}
  \definition{v.}{perceber, tornar-se ciente de | descobrir, encontrar, detectar}
\end{entry}

\begin{entry}{发现者}{fa1xian4 zhe3}{5,8,8}[Radicais ⼜、⾒、⽼]
  \definition{s.}{descobridor}
\end{entry}

\begin{entry}{发言}{fa1yan2}{5,7}[HSK 3][Radicais ⼜、⾔]
  \definition[个]{s.}{discurso; declaração; palestra}
  \definition{v.+compl.}{falar; fazer uma declaração (discurso)}
\end{entry}

\begin{entry}{发音}{fa1yin1}{5,9}[Radicais ⼜、⾳]
  \definition{s.}{pronúncia}
  \definition{v.}{pronunciar}
\end{entry}

\begin{entry}{发展}{fa1zhan3}{5,10}[HSK 3][Radicais ⼜、⼫]
  \definition{s.}{desenvolvimento}
  \definition{v.}{crescer; expandir; avançar; desenvolver | recrutar; expandir; admitir}
\end{entry}

\begin{entry}{罚}{fa2}{9}[Radical ⽹]
  \definition{v.}{castigar | punir}
\end{entry}

\begin{entry}{罚款}{fa2kuan3}{9,12}[Radicais ⽹、⽋]
  \definition{s.}{multa (monetária) | pena}
  \definition{v.+compl.}{aplicar uma multa | multar}
\end{entry}

\begin{entry}{筏}{fa2}{12}[Radical ⽵]
  \definition{s.}{jangada (de troncos, bambus, etc.)}
\end{entry}

\begin{entry}{法}{fa3}{8}[HSK 4][Radical ⽔]
  \definition*{s.}{França, abreviação de~法国 | Doutrina budista; o dharma | sobrenome Fa}
  \definition{s.}{lei; termo geral para regras de comportamento estabelecidas ou endossadas pelo Estado | maneira; método; modo; meios | padrão; modelo | artes mágicas; feitiço}
  \definition{v.}{fazer com que algo pareça, funcione, etc. como outra coisa; imitar; seguir; acompanhar}
  \seealsoref{法国}{fa3guo2}
\end{entry}

\begin{entry}{法官}{fa3 guan1}{8,8}[HSK 4][Radicais ⽔、⼧]
  \definition[位]{s.}{juiz; justiça; termo genérico para um membro do judiciário em um tribunal de justiça}
\end{entry}

\begin{entry}{法国}{fa3guo2}{8,8}[Radicais ⽔、⼞]
  \definition*{s.}{França}
\end{entry}

\begin{entry}{法国人}{fa3guo2ren2}{8,8,2}[Radicais ⽔、⼞、⼈]
  \definition{s.}{francês | pessoa ou povo da França}
\end{entry}

\begin{entry}{法律}{fa3lv4}{8,9}[HSK 4][Radicais ⽔、⼻]
  \definition[项,条,套,个]{s.}{lei; estatuto; regras de conduta formuladas pelo legislativo e cuja aplicação é garantida pelo poder estatal}
\end{entry}

\begin{entry}{法网}{fa3wang3}{8,6}[Radicais ⽔、⽹]
  \definition*{s.}{Torneio de Roland Garros (French Open), torneio de tênis}
\end{entry}

\begin{entry}{法文}{fa3wen2}{8,4}[Radicais ⽔、⽂]
  \definition*{s.}{françês, língua francesa}
\end{entry}

\begin{entry}{法语}{fa3yu3}{8,9}[Radicais ⽔、⾔]
  \definition{s.}{françês, língua francesa}
\end{entry}

\begin{entry}{法院}{fa3yuan4}{8,9}[HSK 4][Radicais ⽔、⾩]
  \definition[所,座]{s.}{tribunal; corte; órgãos estatais que exercem poder judicial independente}
\end{entry}

\begin{entry}{发}{fa4}{5}[Radical ⼜]
  \definition{s.}{cabelo}
  \seeref{发}{fa1}
\end{entry}

\begin{entry}{发型}{fa4xing2}{5,9}[Radicais ⼜、⼟]
  \definition{s.}{penteado}
\end{entry}

\begin{entry}{发簪}{fa4zan1}{5,18}[Radicais ⼜、⽵]
  \definition{s.}{grampo de cabelo}
\end{entry}

\begin{entry}{番茄}{fan1qie2}{12,8}[Radicais ⽥、⾋]
  \definition{s.}{tomate}
\end{entry}

\begin{entry}{蕃茄}{fan1qie2}{15,8}[Radicais ⾋、⾋]
  \variantof{番茄}
\end{entry}

\begin{entry}{翻}{fan1}{18}[HSK 4][Radical ⽻]
  \definition{v.}{virar; dar a volta; inverter; mudar de posição; torcer; reverter | vasculhar; procurar; pesquisar; mover objetos para localizar algo | reverter; retrair; retirar | passar por cima; ultrapassar; cruzar |
multiplicar | traduzir; decodificar | romper-se; cair; desentender-se com alguém}
\end{entry}

\begin{entry}{翻过}{fan1guo4}{18,6}[Radicais ⽻、⾡]
  \definition{v.}{virar |  transformar}
\end{entry}

\begin{entry}{翻脸}{fan1lian3}{18,11}[Radicais ⽻、⾁]
  \definition{v.+compl.}{brigar com alguém | tornar-se hostil}
\end{entry}

\begin{entry}{翻译}{fan1yi4}{18,7}[HSK 4][Radicais ⽻、⾔]
  \definition[个,位,名]{s.}{tradutor; intérprete}
  \definition{v.}{traduzir; interpretar; colocar o significado de palavras de um idioma em palavras de outro idioma (expressão idiomática); expressar um significado em outro idioma}
\end{entry}

\begin{entry}{烦}{fan2}{10}[HSK 4][Radical ⽕]
  \definition{adj.}{(estar) cansado; (estar) aborrecido; irritado; incomodado | supérfluo e confuso, bagunçado}
  \definition{s.}{problema; incômodo}
  \definition{v.}{incomodar; solicitar; colocar alguém em dificuldade (para fazer algo)}
\end{entry}

\begin{entry}{反}{fan3}{4}[HSK 4][Radical ⼜]
  \definition{adj.}{oposto; contrário; invertido | contrarrevolucionário, reacionário}
  \definition{adv.}{pelo contrário; inversamente}
  \definition{v.}{inverter o lado; de cabeça para baixo; na direção oposta | virar; converter | retornar | opor-se; combater; voltar-se contra | rebelar-se; revoltar-se | inferir; deduzir; raciocinar por analogia}
\end{entry}

\begin{entry}{反对}{fan3dui4}{4,5}[HSK 3][Radicais ⼜、⼨]
  \definition{v.}{lutar; opor-se; objetar a; ser contra}
\end{entry}

\begin{entry}{反对党}{fan3dui4dang3}{4,5,10}[Radicais ⼜、⼨、⼉]
  \definition{s.}{partido de oposição}
\end{entry}

\begin{entry}{反对派}{fan3dui4pai4}{4,5,9}[Radicais ⼜、⼨、⽔]
  \definition{s.}{facção de oposição}
\end{entry}

\begin{entry}{反对票}{fan3dui4piao4}{4,5,11}[Radicais ⼜、⼨、⽰]
  \definition{s.}{voto dissidente}
\end{entry}

\begin{entry}{反而}{fan3'er2}{4,6}[HSK 4][Radicais ⼜、⽽]
  \definition{adv.}{em vez disso; ao contrário de; contrário ao significado da frase anterior ou inesperado, desempenha o papel de uma reviravolta em uma frase}
\end{entry}

\begin{entry}{反复}{fan3fu4}{4,9}[HSK 3][Radicais ⼜、⼢]
  \definition{adv.}{repetidamente; de ​​novo e de novo}
  \definition{s.}{reversão; recaída}
  \definition{v.}{recuar; cortar e mudar}
\end{entry}

\begin{entry}{反省}{fan3xing3}{4,9}[Radicais ⼜、⽬]
  \definition{v.}{examinar a consciência | questionar-se | refletir sobre si mesmo | sondar a alma}
\end{entry}

\begin{entry}{反应}{fan3ying4}{4,7}[HSK 3][Radicais ⼜、⼴]
  \definition[个]{s.}{reação; resposta}
  \definition{v.}{reagir; responder}
\end{entry}

\begin{entry}{反映}{fan3ying4}{4,9}[HSK 4][Radicais ⼜、⽇]
  \definition{s.}{reflexão; opiniões sobre pessoas ou situações}
  \definition{v.}{refletir; espelhar; figurativamente, trazer à tona a essência de uma questão objetiva (expressão idiomática); expressar a essência de algo objetivamente | relatar; tornar conhecido; informar às autoridades superiores | refletir; espelhar; a imagem de um objeto aparece invertida em outro objeto}
\end{entry}

\begin{entry}{反正}{fan3zheng4}{4,5}[HSK 3][Radicais ⼜、⽌]
  \definition{adv.}{de qualquer forma | tudo igual; em qualquer caso}
\end{entry}

\begin{entry}{犯法}{fan4fa3}{5,8}[Radicais ⽝、⽔]
  \definition{v.}{violar (quebrar) a lei}
\end{entry}

\begin{entry}{犯罪}{fan4zui4}{5,13}[Radicais ⽝、⽹]
  \definition{v.+compl.}{cometer  um crime (uma ofensa)}
\end{entry}

\begin{entry}{饭}{fan4}{7}[HSK 1][Radical ⾷]
  \definition[碗]{s.}{arroz cozido}
  \definition[顿]{s.}{refeição}
  \definition{s.}{(empréstimo linguístico) fã, devoto}
\end{entry}

\begin{entry}{饭店}{fan4dian4}{7,8}[HSK 1][Radicais ⾷、⼴]
  \definition[家,个]{s.}{restaurante | hotel}
\end{entry}

\begin{entry}{饭馆}{fan4 guan3}{7,11}[HSK 2][Radicais ⾷、⾷]
  \definition[家,个]{s.}{restaurante | lanchonete}
\end{entry}

\begin{entry}{范围}{fan4wei2}{9,7}[HSK 3][Radicais ⾋、⼞]
  \definition[个]{s.}{escopo; limite; alcance}
  \definition{v.}{estabelecer limites para; limitar o escopo de}
\end{entry}

\begin{entry}{方}{fang1}{4}[HSK 4][Radical ⽅]
  \definition*{s.}{sobrenome Fang | Alquimia ``方术''}
  \definition{adj.}{reto; honesto; imparcial}
  \definition{adv.}{exatamente quando; no momento em que}
  \definition{clas.}{para coisas quadradas | quadrado ou cúbico (geralmente metro quadrado ou cúbico)}
  \definition{s.}{quadrado; um quadrado ou sólido com seis faces quadradas | (matemática) potência; o número de vezes que uma quantidade deve ser multiplicada por si mesma | direção | lado; festa | lugar; região; localidade | maneira; método; solução | prescrição | lei; regra}
  \seealsoref{方术}{fang1 shu4}
\end{entry}

\begin{entry}{方案}{fang1'an4}{4,10}[HSK 4][Radicais ⽅、⽊]
  \definition[个,套]{s.}{plano; esquema; programa; planos específicos para tratar de um determinado problema | o esquema criado pelo governo; medidas ou regulamentações formuladas e implementadas pelo governo ou autoridades relevantes}
\end{entry}

\begin{entry}{方便}{fang1bian4}{4,9}[HSK 2][Radicais ⽅、⼈]
  \definition{adj.}{conveniente | adequado}
  \definition{v.}{facilitar, facilitar as coisas | ter dinheiro de sobra | (eufemismo) aliviar-se}
\end{entry}

\begin{entry}{方便面}{fang1 bian4 mian4}{4,9,9}[HSK 2][Radicais ⽅、⼈、⾯]
  \definition{s.}{macarrão instantâneo}
\end{entry}

\begin{entry}{方法}{fang1fa3}{4,8}[HSK 2][Radicais ⽅、⽔]
  \definition[个]{s.}{método | meio}
\end{entry}

\begin{entry}{方面}{fang1mian4}{4,9}[HSK 2][Radicais ⽅、⾯]
  \definition[个]{s.}{lado | campo | aspecto}
\end{entry}

\begin{entry}{方式}{fang1shi4}{4,6}[HSK 3][Radicais ⽅、⼷]
  \definition[种,个]{s.}{maneira; método}
\end{entry}

\begin{entry}{方术}{fang1 shu4}{4,5}[Radicais ⽅、⽊]
  \definition{s.}{artes de cura, adivinhação, horóscopo etc. | artes sobrenaturais (antigo)}
\end{entry}

\begin{entry}{方向}{fang1xiang4}{4,6}[HSK 2][Radicais ⽅、⼝]
  \definition[个]{s.}{direção | orientação | alvo | meta | objetivo}
\end{entry}

\begin{entry}{方言}{fang1yan2}{4,7}[Radicais ⽅、⾔]
  \definition*{s.}{o primeiro dicionário de dialeto chinês, editado por Yang Xiong 扬雄 no século I, contendo mais de 9.000 caracteres}
  \definition{s.}{dialeto}
  \seealsoref{扬雄}{yang2xiong2}
\end{entry}

\begin{entry}{方针}{fang1zhen1}{4,7}[HSK 4][Radicais ⽅、⾦]
  \definition[个]{s.}{política; diretriz; princípio orientador; orientação da direção e das metas de um empreendimento}
\end{entry}

\begin{entry}{防}{fang2}{6}[HSK 3][Radical ⾩]
  \definition*{s.}{sobrenome Fang}
  \definition{s.}{defesa | barragem; dique; aterro}
  \definition{v.}{prover contra; defender contra; proteger contra}
\end{entry}

\begin{entry}{防护}{fang2hu4}{6,7}[Radicais ⾩、⼿]
  \definition{v.}{defender | proteger}
\end{entry}

\begin{entry}{防晒}{fang2shai4}{6,10}[Radicais ⾩、⽇]
  \definition{s.}{protetor solar}
\end{entry}

\begin{entry}{防止}{fang2zhi3}{6,4}[HSK 3][Radicais ⾩、⽌]
  \definition{v.}{evitar; prevenir; prevenir; proteger contra}
\end{entry}

\begin{entry}{房东}{fang2dong1}{8,5}[HSK 3][Radicais ⼾、⼀]
  \definition[个,位]{s.}{dono;  proprietário; senhorio}
\end{entry}

\begin{entry}{房间}{fang2jian1}{8,7}[HSK 1][Radicais ⼾、⾨]
  \definition[间,个]{s.}{quarto}
\end{entry}

\begin{entry}{房屋}{fang2 wu1}{8,9}[HSK 3][Radicais ⼾、⼫]
  \definition[间,所,套]{s.}{casas; habitação; edifícios}
\end{entry}

\begin{entry}{房主}{fang2zhu3}{8,5}[Radicais ⼾、⼂]
  \definition{s.}{proprietário | dono de um imóvel}
\end{entry}

\begin{entry}{房子}{fang2 zi5}{8,3}[HSK 1][Radicais ⼾、⼦]
  \definition[栋,幢,座,套,间,个]{s.}{apartamento | casa | quarto}
\end{entry}

\begin{entry}{房租}{fang2 zu1}{8,10}[HSK 3][Radicais ⼾、⽲]
  \definition[笔]{s.}{aluguel}
\end{entry}

\begin{entry}{访问}{fang3wen4}{6,6}[HSK 3][Radicais ⾔、⾨]
  \definition{v.}{visitar; ligar; entrevistar | visitar um \emph{site}}
\end{entry}

\begin{entry}{放}{fang4}{8}[HSK 1][Radical ⽅]
  \definition{v.}{liberar | libertar | deixar ir | colocar | por | detonar (fogos de artifício)}
\end{entry}

\begin{entry}{放鞭炮}{fang4bian1pao4}{8,18,9}[Radicais ⽅、⾰、⽕]
  \definition{s.}{um conjunto de bombinhas ou traques}
\end{entry}

\begin{entry}{放出}{fang4chu1}{8,5}[Radicais ⽅、⼐]
  \definition{v.}{liberar | libertar}
\end{entry}

\begin{entry}{放大}{fang4da4}{8,3}[Radicais ⽅、⼤]
  \definition{v.}{ampliar}
\end{entry}

\begin{entry}{放到}{fang4 dao4}{8,8}[HSK 3][Radicais ⽅、⼑]
  \definition{v.}{colocar em; meter}
\end{entry}

\begin{entry}{放电}{fang4dian4}{8,5}[Radicais ⽅、⽥]
  \definition{s.}{descarga elétrica}
\end{entry}

\begin{entry}{放飞}{fang4fei1}{8,3}[Radicais ⽅、⾶]
  \definition{s.}{deixar voar}
\end{entry}

\begin{entry}{放过}{fang4guo4}{8,6}[Radicais ⽅、⾡]
  \definition{v.}{deixar | deixar alguém escapar impune | passar despercebido}
\end{entry}

\begin{entry}{放假}{fang4 jia4}{8,11}[HSK 1][Radicais ⽅、⼈]
  \definition{v.}{ter férias ou feriado}
\end{entry}

\begin{entry}{放弃}{fang4qi4}{8,7}[Radicais ⽅、⼶]
  \definition{v.}{abandonar | desistir de | renunciar}
\end{entry}

\begin{entry}{放弃权利}{fang4qi4 quan2li4}{8,7,6,7}[Radicais ⽅、⼶、⽊、⼑]
  \definition{s.}{renúncia}
\end{entry}

\begin{entry}{放弃者}{fang4qi4zhe3}{8,7,8}[Radicais ⽅、⼶、⽼]
  \definition{s.}{desistente}
\end{entry}

\begin{entry}{放任}{fang4ren4}{8,6}[Radicais ⽅、⼈]
  \definition{v.}{ignorar | saciar-se | deixar sozinho}
\end{entry}

\begin{entry}{放肆}{fang4si4}{8,13}[Radicais ⽅、⾀]
  \definition{adj.}{atrevido | pesunçoso | devasso}
\end{entry}

\begin{entry}{放松}{fang4song1}{8,8}[HSK 4][Radicais ⽅、⽊]
  \definition{adj.}{relaxado; afrouxado; solto; desprendido}
  \definition{v.}{relaxar; afrouxar; soltar; desprender}
\end{entry}

\begin{entry}{放下}{fang4 xia4}{8,3}[HSK 2][Radicais ⽅、⼀]
  \definition{v.}{deitar | colocar para baixo | deixar ir | liberar | desistir | colocar em algum lugar}
\end{entry}

\begin{entry}{放心}{fang4xin1}{8,4}[HSK 2][Radicais ⽅、⼼]
  \definition{adj.}{despreocupado}
  \definition{v.}{sentir-se aliviado | sentir-se tranquilo | ficar à vontade}
  \definition{v.+compl.}{confiar | ter confiança em alguém | estar à vontade | sentir-se aliviado}
\end{entry}

\begin{entry}{放学}{fang4 xue2}{8,8}[HSK 1][Radicais ⽅、⼦]
  \definition{v.+compl.}{sair da escola | acabar as aulas | terminar a aula (por hoje)}
\end{entry}

\begin{entry}{放养}{fang4yang3}{8,9}[Radicais ⽅、⼋]
  \definition{v.}{criar (gado, peixes, culturas, etc.) | crescer | criar}
\end{entry}

\begin{entry}{放走}{fang4zou3}{8,7}[Radicais ⽅、⾛]
  \definition{v.}{permitir (uma pessoa ou um animal) ir | liberar | libertar}
\end{entry}

\begin{entry}{飞}{fei1}{3}[HSK 1][Radical ⾶]
  \definition*{s.}{sobrenome Fei}
  \definition{adj.}{inesperado | acidental | infundado | sem fundamento}
  \definition{adv.}{rapidamente}
  \definition{s.}{roda livre de uma bicicleta}
  \definition{v.}{voar | esvoaçar | flutuar no ar | volatilizar}
\end{entry}

\begin{entry}{飞船}{fei1chuan2}{3,11}[Radicais ⾶、⾈]
  \definition{s.}{espaçonave | dirigível | aeronave}
\end{entry}

\begin{entry}{飞碟}{fei1die2}{3,14}[Radicais ⾶、⽯]
  \definition{s.}{disco-voador, OVNI, \emph{UFO} | \emph{frisbee}}
\end{entry}

\begin{entry}{飞机}{fei1ji1}{3,6}[HSK 1][Radicais ⾶、⽊]
  \definition[架]{s.}{avião}
\end{entry}

\begin{entry}{飞机票}{fei1ji1 piao4}{3,6,11}[Radicais ⾶、⽊、⽰]
  \definition[张]{s.}{bilhete de avião}
  \seealsoref{机票}{ji1 piao4}
\end{entry}

\begin{entry}{飞行}{fei1 xing2}{3,6}[HSK 3][Radicais ⾶、⾏]
  \definition{s.}{voo | aviação}
  \definition{v.}{voar; fazer um voo | (aviões, foguetes, etc.) voar no ar}
\end{entry}

\begin{entry}{非}{fei1}{8}[HSK 4][Kangxi 175][Radical ⾮]
  \definition*{s.}{África, abreviação de 非洲 | sobrenome Fei}
  \definition{adv.}{Em resposta a ``不'', indica necessidade (deve)}
  \definition{pref.}{indicando negatividade ou exclusão}
  \definition{s.}{engano; erro}
  \definition{v.}{opor-se a; culpar; censurar | não estar em conformidade com; ser contrário a | não ser | ter que; simplesmente precisar (fazer algo)}
  \seealsoref{不}{bu4}
  \seealsoref{非洲}{fei1zhou1}
\end{entry}

\begin{entry}{非常}{fei1chang2}{8,11}[HSK 1][Radicais ⾮、⼱]
  \definition{adv.}{extraordinário | altamente | muito}
\end{entry}

\begin{entry}{非洲}{fei1zhou1}{8,9}[Radicais ⾮、⽔]
  \definition*{s.}{África}
\end{entry}

\begin{entry}{非洲人}{fei1zhou1ren2}{8,9,2}[Radicais ⾮、⽔、⼈]
  \definition{s.}{africano | pessoa ou povo da África}
\end{entry}

\begin{entry}{肥}{fei2}{8}[HSK 4][Radical ⾁]
  \definition*{s.}{sobrenome Fei}
  \definition{adj.}{gordo; gorduroso | fértil; rico | solto e largo; folgado | inchaço (de órgão interno)}
  \definition{s.}{fertilizante; esterco}
  \definition{v.}{fertilizar; adubar | enriquecer com renda ilegal, ilícita}
\end{entry}

\begin{entry}{狒狒}{fei4fei4}{8,8}[Radicais ⽝、⽝]
  \definition{s.}{babuíno}
\end{entry}

\begin{entry}{费}{fei4}{9}[HSK 3][Radical ⾙]
  \definition*{s.}{Fei}
  \definition{s.}{taxa; despesa; encargo}
  \definition{v.}{custar; gastar; desperdiçar}
\end{entry}

\begin{entry}{费用}{fei4 yong4}{9,5}[HSK 3][Radicais ⾙、⽤]
  \definition[笔,个]{s.}{custo; despesa; desembolso}
\end{entry}

\begin{entry}{分}{fen1}{4}[HSK 1,2][Radical ⼑]
  \definition{s.}{parte ou subdivisão | fração | um décimo (de certas unidades) | unidade de comprimento equivalente a 0,33cm | minuto (unidade de tempo) | minuto (unidade de medida angular) | um ponto (em esportes e jogos) | 0,01 yuan (unidade de dinheiro)}
  \definition{v.}{dividir | separar | distribuir | atribuir | distinguir (bom e mau)}
  \seeref{分}{fen4}
\end{entry}

\begin{entry}{分别}{fen1bie2}{4,7}[HSK 3][Radicais ⼑、⼑]
  \definition{adv.}{diferentemente; de maneiras diferentes}
  \definition{s.}{diferença}
  \definition{v.}{partir; deixar um ao outro | distinguir; diferenciar}
\end{entry}

\begin{entry}{分布}{fen1bu4}{4,5}[HSK 4][Radicais ⼑、⼱]
  \definition{v.}{espalhar; distribuir; dispersar (em uma determinada área)}
\end{entry}

\begin{entry}{分公司}{fen1gong1si1}{4,4,5}[Radicais ⼑、⼋、⼝]
  \definition{s.}{sucursal | filial de companhia}
\end{entry}

\begin{entry}{分开}{fen1 kai1}{4,4}[HSK 2][Radicais ⼑、⼶]
  \definition{v.+compl.}{separar | dividir | desacoplar | desempacotar | quebrar | desmembrar | romper | desfazer | desvincular | distribuir | separar de (em) | dividir \dots de \dots | separar de}
\end{entry}

\begin{entry}{分量}{fen1liang4}{4,12}[Radicais ⼑、⾥]
  \definition{s.}{componente vetorial}
  \seeref{分量}{fen4liang4}
  \seeref{分量}{fen4liang5}
\end{entry}

\begin{entry}{分配}{fen1pei4}{4,10}[HSK 3][Radicais ⼑、⾣]
  \definition{v.}{atribuir; dispor | atribuir; compartilhar; distribuir}
\end{entry}

\begin{entry}{分散}{fen1san4}{4,12}[HSK 4][Radicais ⼑、⽁]
  \definition{adj.}{espalhado; disperso; desviado; fragmentado; sem foco}
  \definition{v.}{dispersar; espalhar; descentralizar | separar-se; desunir-se}
\end{entry}

\begin{entry}{分手}{fen1shou3}{4,4}[HSK 4][Radicais ⼑、⼿]
  \definition{v.+compl.}{separar; romper; terminar um relacionamento ou um casal | separar-se (de uma empresa); dizer adeus; despedir-se da família, dos amigos, etc.}
\end{entry}

\begin{entry}{分数}{fen1 shu4}{4,13}[HSK 2][Radicais ⼑、⽁]
  \definition{s.}{fração | número fracionário | marca | nota | ponto}
\end{entry}

\begin{entry}{分为}{fen1 wei2}{4,4}[HSK 4][Radicais ⼑、⼂]
  \definition{v.}{subdividir; dividir algo em}
\end{entry}

\begin{entry}{分之}{fen1 zhi1}{4,3}[HSK 4][Radicais ⼑、⼂]
  \definition{expr.}{indicando uma fração; formatação e leitura de frações, ou seja, partes de um total}
  \example{顾客减少了三分之一。}[O número de clientes caiu em um terço.]
\end{entry}

\begin{entry}{分钟}{fen1zhong1}{4,9}[HSK 2][Radicais ⼑、⾦]
  \definition{s.}{minuto (usado em intervalos de tempo)}
\end{entry}

\begin{entry}{分子}{fen1zi3}{4,3}[Radicais ⼑、⼦]
  \definition{s.}{molécula | (matemática) numerador de uma fração}
  \seeref{分子}{fen4zi3}
\end{entry}

\begin{entry}{分组}{fen1 zu3}{4,8}[HSK 3][Radicais ⼑、⽷]
  \definition{v.}{agrupar; dividir em grupos}
\end{entry}

\begin{entry}{纷纷}{fen1fen1}{7,7}[HSK 4][Radicais ⽷、⽷]
  \definition{adj.}{numeroso e confuso; muitos e desordenados}
  \definition{adv.}{um após o outro; em sucessão; em rápida sucessão}
\end{entry}

\begin{entry}{焚香}{fen2xiang1}{12,9}[Radicais ⽕、⾹]
  \definition{v.}{queimar incenso}
\end{entry}

\begin{entry}{粉}{fen3}{10}[Radical ⽶]
  \definition{s.}{pó | pó cosmético facial | alimento preparado a partir de amido | macarrão feito de qualquer tipo de farinha}
  \definition{v.}{tornar algo em pó | ser um fã de}
\end{entry}

\begin{entry}{粉色}{fen3 se4}{10,6}[Radicais ⽶、⾊]
  \definition{s.}{cor-de-rosa}
\end{entry}

\begin{entry}{粉丝}{fen3si1}{10,5}[Radicais ⽶、⼀]
  \definition{s.}{(empréstimo linguístico) fã | entusiasta de alguém ou alguma coisa}
  \definition[把]{s.}{aletria de amido de feijão | aletria chinesa | macarrão de celofane ou macarrão de vidro (transparente)}
\end{entry}

\begin{entry}{分}{fen4}{4}[HSK 2][Radical ⼑]
  \definition{s.}{parte | ingrediente | componente}
  \seeref{分}{fen1}
\end{entry}

\begin{entry}{分量}{fen4liang4}{4,12}[Radicais ⼑、⾥]
  \definition{s.}{tamanho da porção (comida)}
  \seeref{分量}{fen1liang4}
  \seeref{分量}{fen4liang5}
\end{entry}

\begin{entry}{分量}{fen4liang5}{4,12}[Radicais ⼑、⾥]
  \definition{s.}{quantidade | peso | medida}
  \seeref{分量}{fen1liang4}
  \seeref{分量}{fen4liang4}
\end{entry}

\begin{entry}{分子}{fen4zi3}{4,3}[Radicais ⼑、⼦]
  \definition{s.}{membros de uma classe ou grupo | elementos políticos (como intelectuais ou extremistas)}
  \seeref{分子}{fen1zi3}
\end{entry}

\begin{entry}{份}{fen4}{6}[HSK 2][Radical ⼈]
  \definition{clas.}{para presentes, jornais, revistas, papéis, relatórios, contratos, etc. ou pratos (refeição)}
\end{entry}

\begin{entry}{奋斗}{fen4dou4}{8,4}[HSK 4][Radicais ⼤、⽃]
  \definition{v.}{lutar; esforçar-se; batalhar; trabalhar duro para atingir um determinado objetivo}
\end{entry}

\begin{entry}{奋战}{fen4zhan4}{8,9}[Radicais ⼤、⼽]
  \definition{v.}{lutar bravamente | trabalhar duro}
\end{entry}

\begin{entry}{愤怒}{fen4nu4}{12,9}[Radicais ⼼、⼼]
  \definition{adj.}{zangado | indignado}
  \definition{s.}{ira}
\end{entry}

\begin{entry}{愤世嫉俗}{fen4shi4ji2su2}{12,5,13,9}[Radicais ⼼、⼀、⼥、⼈]
  \definition{v.}{ser cínico | ser amargurado}
\end{entry}

\begin{entry}{丰富}{feng1fu4}{4,12}[HSK 3][Radicais ⼁、⼧]
  \definition{adj.}{rico; abundante; pleno}
  \definition{v.}{enriquecer}
\end{entry}

\begin{entry}{丰收}{feng1shou1}{4,6}[Radicais ⼁、⽁]
  \definition{s.}{colheita abundante}
\end{entry}

\begin{entry}{风}{feng1}{4}[HSK 1][Kangxi 182][Radical ⾵]
  \definition[阵,丝]{s.}{vento}
\end{entry}

\begin{entry}{风格}{feng1ge2}{4,10}[HSK 4][Radicais ⾵、⽊]
  \definition{s.}{modo; estilo; maneira; caráter | características das criações literárias de diferentes épocas, povos, escolas ou indivíduos em termos de conteúdo ideológico e técnicas artísticas}
\end{entry}

\begin{entry}{风景}{feng1jing3}{4,12}[HSK 4][Radicais ⾵、⽇]
  \definition[种,处,道]{s.}{cenário; paisagem; cenários e vistas que podem ser apreciados, inclui paisagens, flores, árvores, edifícios e determinados fenômenos naturais}
\end{entry}

\begin{entry}{风扇}{feng1shan4}{4,10}[Radicais ⾵、⼾]
  \definition{s.}{ventilador elétrico}
\end{entry}

\begin{entry}{风俗}{feng1su2}{4,9}[HSK 4][Radicais ⾵、⼈]
  \definition[种,个,些]{s.}{costumes; a soma de costumes sociais, maneiras, hábitos, etc., desenvolvidos ao longo do tempo.}
\end{entry}

\begin{entry}{风险}{feng1xian3}{4,9}[HSK 3][Radicais ⾵、⾩]
  \definition[个,种,项,类]{s.}{risco; perigo}
\end{entry}

\begin{entry}{风筝}{feng1zheng5}{4,12}[Radicais ⾵、⽵]
  \definition{s.}{pipa | papagaio | pandorga}
\end{entry}

\begin{entry}{枫叶}{feng1ye4}{8,5}[Radicais ⽊、⼝]
  \definition{s.}{folha de bordo (maple, tipo de árvore)}
\end{entry}

\begin{entry}{封}{feng1}{9}[HSK 2][Radical ⼨]
  \definition*{s.}{sobrenome Feng}
  \definition{clas.}{para objetos selados, especialmente cartas}
  \definition{v.}{conceder um título | conferir | conceder | selar}
\end{entry}

\begin{entry}{封闭}{feng1bi4}{9,6}[HSK 4][Radicais ⼨、⾨]
  \definition{adj.}{fechado; aqueles que não têm contato com o mundo exterior; aqueles que são muito conservadores (em seu pensamento) e não se comunicam com os outros}
  \definition{v.}{selar; fechar; lacrar; vedar; de modo a impedir a passagem, o uso ou a abertura}
\end{entry}

\begin{entry}{封底}{feng1di3}{9,8}[Radicais ⼨、⼴]
  \definition{s.}{contracapa de um livro}
\end{entry}

\begin{entry}{封冻}{feng1dong4}{9,7}[Radicais ⼨、⼎]
  \definition{v.}{congelar (água ou terra)}
\end{entry}

\begin{entry}{封盖}{feng1gai4}{9,11}[Radicais ⼨、⽫]
  \definition{s.}{boné | capa | selo}
  \definition{v.}{cobrir}
\end{entry}

\begin{entry}{封建}{feng1jian4}{9,8}[Radicais ⼨、⼵]
  \definition{adj.}{feudal}
  \definition{s.}{feudalismo}
\end{entry}

\begin{entry}{封口}{feng1kou3}{9,3}[Radicais ⼨、⼝]
  \definition{v.}{selar | fechar | curar (uma ferida) | manter os lábios selados}
\end{entry}

\begin{entry}{封面}{feng1mian4}{9,9}[Radicais ⼨、⾯]
  \definition{s.}{capa (de uma publicação) | sobrecapa}
\end{entry}

\begin{entry}{封印}{feng1yin4}{9,5}[Radicais ⼨、⼙]
  \definition{s.}{selo (em envelopes)}
\end{entry}

\begin{entry}{封斋}{feng1zhai1}{9,10}[Radicais ⼨、⽂]
  \definition*{s.}{Ramadã (Islã)}
\end{entry}

\begin{entry}{疯狂}{feng1kuang2}{9,7}[Radicais ⽧、⽝]
  \definition{adj.}{louco | frenético | selvagem}
\end{entry}

\begin{entry}{缝纫}{feng2ren4}{13,6}[Radicais ⽷、⽷]
  \definition{v.}{costurar}
\end{entry}

\begin{entry}{缝纫机}{feng2ren4ji1}{13,6,6}[Radicais ⽷、⽷、⽊]
  \definition[架]{s.}{máquina de costura}
\end{entry}

\begin{entry}{凤凰}{feng4huang2}{4,11}[Radicais ⼏、⼏]
  \definition{s.}{fênix}
\end{entry}

\begin{entry}{佛}{fo2}{7}[Radical ⼈]
  \definition*{s.}{Buda, abreviação de 佛陀 | Budismo}
  \seeref{佛}{fu2}
  \seealsoref{佛陀}{fo2tuo2}
\end{entry}

\begin{entry}{佛陀}{fo2tuo2}{7,7}[Radicais ⼈、⾩]
  \definition{s.}{Buda (uma pessoa que atingiu a Budeidade, ou especificamente Siddhartha Gautama)}
\end{entry}

\begin{entry}{否定}{fou3ding4}{7,8}[HSK 3][Radicais ⼝、⼧]
  \definition{adj.}{negativo}
  \definition{s.}{negativo (resposta); negação}
  \definition{v.}{rejeitar; negar}
\end{entry}

\begin{entry}{否认}{fou3ren4}{7,4}[HSK 3][Radicais ⼝、⾔]
  \definition{v.}{negar; repudiar}
\end{entry}

\begin{entry}{否则}{fou3ze2}{7,6}[HSK 4][Radicais ⼝、⼑]
  \definition{conj.}{senão; se não; ou então; se não for isso}
\end{entry}

\begin{entry}{夫妇}{fu1fu4}{4,6}[HSK 4][Radicais ⼤、⼥]
  \definition[对]{s.}{casal; marido e mulher}
\end{entry}

\begin{entry}{夫妻}{fu1qi1}{4,8}[HSK 4][Radicais ⼤、⼥]
  \definition[对]{s.}{casal; marido e mulher}
\end{entry}

\begin{entry}{夫人}{fu1ren2}{4,2}[HSK 4][Radicais ⼤、⼈]
  \definition[位]{s.}{senhora; \emph{lady}; madame; na antiguidade, as esposas dos senhores feudais eram chamadas de ``madame'' e, nas dinastias Ming e Qing, as esposas dos oficiais de primeiro e segundo escalão eram chamadas de ``madame'', que mais tarde foi usada para homenagear as esposas das pessoas em geral e agora é usada principalmente em ocasiões diplomáticas}
\end{entry}

\begin{entry}{佛}{fu2}{7}[Radical ⼈]
  \definition{adv.}{aparentemente}
  \definition{s.}{ornamento da cabeça (feminino)}
  \seeref{佛}{fo2}
\end{entry}

\begin{entry}{扶梯}{fu2ti1}{7,11}[Radicais ⼿、⽊]
  \definition{s.}{escada rolante}
\end{entry}

\begin{entry}{服}{fu2}{8}[Radical ⽉]
  \definition{s.}{roupas | vestido | vestuário | roupa de luto}
  \definition{v.}{servir (nas forças armadas, uma sentença de prisão, etc.) | obedecer | ser convencido (por um argumento) | convencer | admirar | aclimatar | tomar (medicamento) | usar roupas de luto}
  \seeref{服}{fu4}
\end{entry}

\begin{entry}{服务}{fu2 wu4}{8,5}[HSK 2][Radicais ⽉、⼒]
  \definition{v.}{prestar serviço a | estar a serviço de | servir | trabalhar | servir}
\end{entry}

\begin{entry}{服务员}{fu2wu4yuan2}{8,5,7}[Radicais ⽉、⼒、⼝]
  \definition{s.}{atendente | garçom | garçonete | pessoal de atendimento ao cliente}
\end{entry}

\begin{entry}{服装}{fu2zhuang1}{8,12}[HSK 3][Radicais ⽉、⾐]
  \definition[套,件,身]{s.}{roupas; trajes; fantasias}
\end{entry}

\begin{entry}{浮力}{fu2li4}{10,2}[Radicais ⽔、⼒]
  \definition{s.}{flutuabilidade}
\end{entry}

\begin{entry}{浮图}{fu2tu2}{10,8}[Radicais ⽔、⼞]
  \definition*{s.}{Termo alternativo para 佛陀}
  \variantof{浮屠}
  \seealsoref{佛陀}{fo2tuo2}
\end{entry}

\begin{entry}{浮屠}{fu2tu2}{10,11}[Radicais ⽔、⼫]
  \definition*{s.}{Buda | Templo (Stupa) Budista (transliteração de Pali Thuo)}
\end{entry}

\begin{entry}{符号}{fu2hao4}{11,5}[HSK 4][Radicais ⽵、⼝]
  \definition[个]{s.}{marca; símbolo; sinais que marcam as coisas
insígnia; emblema; um símbolo usado no corpo para indicar posição, \emph{status}, etc.}
\end{entry}

\begin{entry}{符合}{fu2he2}{11,6}[HSK 4][Radicais ⽵、⼝]
  \definition{conj.}{de acordo com; concordando com; contando com; alinhado com}
  \definition{v.}{concordar com; estar em conformidade com; corresponder com | gerenciar; lidar}
\end{entry}

\begin{entry}{福}{fu2}{13}[HSK 3][Radical ⽰]
  \definition*{s.}{sobrenome Fu}
  \definition{s.}{benção; felicidade; boa sorte; boa fortuna}
  \definition{v.}{curvar-se; reverenciar}
\end{entry}

\begin{entry}{福克斯}{fu2ke4si1}{13,7,12}[Radicais ⽰、⼗、⽄]
  \definition*{s.}{Fox (empresa de mídia) | Focus (automóvel fabricado pela Ford)}
\end{entry}

\begin{entry}{福泽}{fu2ze2}{13,8}[Radicais ⽰、⽔]
  \definition{s.}{boa sorte}
\end{entry}

\begin{entry}{父母}{fu4 mu3}{4,5}[HSK 3][Radicais ⽗、⽏]
  \definition{s.}{pai e mãe; pais}
\end{entry}

\begin{entry}{父母亲}{fu4mu3qin1}{4,5,9}[Radicais ⽗、⽏、⼇]
  \definition{s.}{pais}
\end{entry}

\begin{entry}{父亲}{fu4qin1}{4,9}[HSK 3][Radicais ⽗、⼇]
  \definition[个,位]{s.}{pai}
\end{entry}

\begin{entry}{付}{fu4}{5}[HSK 3][Radical ⼈]
  \definition*{s.}{sobrenome Fu}
  \definition{clas.}{para pares ou conjuntos de coisas | para expressões faciais}
  \definition{v.}{comprometer-se a; entregar (entregar) a; entregar | pagar}
\end{entry}

\begin{entry}{付出}{fu4 chu1}{5,5}[HSK 4][Radicais ⼈、⼐]
  \definition{v.}{pagar; gastar; entregar (dinheiro, consideração, etc.)}
\end{entry}

\begin{entry}{付款}{fu4kuan3}{5,12}[Radicais ⼈、⽋]
  \definition{s.}{pagamento}
  \definition{v.+compl.}{pagar uma quantia em dinheiro}
\end{entry}

\begin{entry}{负担}{fu4dan1}{6,8}[HSK 4][Radicais ⾙、⼿]
  \definition{s.}{carga; fardo; frete; ônus; pressão ou responsabilidade, despesas, etc.}
  \definition{v.}{carregar; carregar (um fardo); assumir (responsabilidade, trabalho, despesas, etc.)}
\end{entry}

\begin{entry}{负责}{fu4ze2}{6,8}[HSK 3][Radicais ⾙、⾙]
  \definition{adj.}{consciencioso}
  \definition{v.}{ser responsável por; estar encarregado de}
\end{entry}

\begin{entry}{附近}{fu4jin4}{7,7}[HSK 4][Radicais ⾩、⾡]
  \definition{adj.}{perto; vizinho}
  \definition{s.}{vizinhança; bairro}
\end{entry}

\begin{entry}{服}{fu4}{8}[Radical ⽉]
  \definition{clas.}{(para remédio) dose}
  \seeref{服}{fu2}
\end{entry}

\begin{entry}{复活节}{fu4huo2jie2}{9,9,5}[Radicais ⼢、⽔、⾋]
  \definition*{s.}{Páscoa}
\end{entry}

\begin{entry}{复刻}{fu4ke4}{9,8}[Radicais ⼢、⼑]
  \definition{v.}{reimprimir (um trabalho que esteve fora do catálogo) | reeditar (um disco de vinil, um CD, etc.) | replicar | recriar | (empréstimo linguístico) (computação) \emph{fork}}
\end{entry}

\begin{entry}{复习}{fu4xi2}{9,3}[HSK 2][Radicais ⼢、⼄]
  \definition{s.}{revisão}
  \definition{v.}{rever | revisar}
\end{entry}

\begin{entry}{复印}{fu4yin4}{9,5}[HSK 3][Radicais ⼢、⼙]
  \definition{v.}{fotografar; fotocopiar; duplicar}
\end{entry}

\begin{entry}{复杂}{fu4za2}{9,6}[HSK 3][Radicais ⼢、⽊]
  \definition{adj.}{complexo; complicado}
\end{entry}

\begin{entry}{复制}{fu4zhi4}{9,8}[HSK 4][Radicais ⼢、⼑]
  \definition{v.}{copiar; duplicar; reproduzir; fazer uma cópia de; fazer uma cópia do original ou reproduzi-lo, reimprimi-lo ou copiá-lo em sua forma original (geralmente referindo-se a relíquias culturais ou obras de arte)}
\end{entry}

\begin{entry}{副}{fu4}{11}[Radical ⼑]
  \definition{clas.}{para pares, conjuntos de coisas e expressões faciais | para óculos, luvas, etc.}
\end{entry}

\begin{entry}{富}{fu4}{12}[HSK 3][Radical ⼧]
  \definition*{s.}{sobrenome Fu}
  \definition{adj.}{rico; póspero | rico; abundante}
  \definition{s.}{fortuna; riqueza}
\end{entry}

\begin{entry}{覆盆子}{fu4pen2zi5}{18,9,3}[Radicais ⾑、⽫、⼦]
  \definition{s.}{framboesa}
\end{entry}

%%%%% EOF %%%%%

