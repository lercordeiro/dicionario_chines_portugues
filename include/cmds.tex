...
%%% Adiciona os antônumos e sinônimos ao dicionário
\newbool{withexamples}
\newbool{withantonyms}
\newbool{withsynonyms}

\booltrue{withexamples}
\booltrue{withantonyms}
\booltrue{withsynonyms}
...
\ifdraftdoc
 \makeevenfoot{plain}{\thepage}{汉葡词典}{Rascunho}
 \makeoddfoot{plain}{Rascunho}{汉葡词典}{\thepage}
\else
...
\ifdraftdoc
 \makeevenhead{dictionary}{\rightmark}{Rascunho}{\leftmark}
 \makeoddhead{dictionary}{\rightmark}{Rascunho}{\leftmark}
 \makeevenfoot{dictionary}{\thepage}{汉葡词典}{Rascunho}
 \makeoddfoot{dictionary}{Rascunho}{汉葡词典}{\thepage}
\else
...
% Cria a lista de Exemplos como uma lista numerada com letras de a-z
\makeatletter
...
\setlist[seelist]{label={\roman*)},left=0pt,topsep=0pt,font=\bfseries,itemjoin={, },itemjoin*={ e },nosep}
\setlist[seealsolist]{label={\roman*)},left=0pt,topsep=0pt,font=\bfseries,itemjoin={, },itemjoin*={ e },nosep}
\setlist[synonymlist]{label={\roman*)},left=0pt,topsep=0pt,font=\bfseries,itemjoin={, },itemjoin*={ e },nosep}
\setlist[antonymlist]{label={\roman*)},left=0pt,topsep=0pt,font=\bfseries,itemjoin={, },itemjoin*={ e },nosep}
...
%%% com os pinyins ao lado dos caracteres
\newcommand{\areference}[2]
 {
  ...
  \StrLen{#2}[\plen]%
   \ifnumcomp{\hlen+\plen}{>}{24}
    {%
     \hyperref[\l_label_tl : #1 : #2]{\foreignlanguage{chinese-hans}{#1}\ (p.~\pageref*{\l_label_tl : #1 : #2})\newline{\footnotesize\dpy{#2}}}
    }
    {%
     \hyperref[\l_label_tl : #1 : #2]{\foreignlanguage{chinese-hans}{#1}\ {\footnotesize\dpy{#2}}\ (p.~\pageref*{\l_label_tl : #1 : #2})}
    }
 }
...
\NewDocumentCommand{\variantof}{m}
 {
  {\small\ding{108}}\ Variante\ de\ \hyperref[\l_label_tl : #1 : \l_pinyin_tl]{\foreignlanguage{chinese-hans}{#1}\ (p.~\pageref*{\l_label_tl : #1 : \l_pinyin_tl})}\par
 }
...
\NewDocumentEnvironment{EntryWithPhonetic}{mO{}mO{}mmooo}%
 {%
  ...
  \markboth{#1\ {\small\dpy{#3}}}{#1\ {\small\dpy{#3}}}
...
\NewDocumentEnvironment{Entry}{mmmooo}%
 {%
  ...
  \IfStrEq{\l_label_tl}{strokes}{\markboth{#1\ {\small(#2画)}}{#1\ {\small(#2画)}}}{}
  \IfStrEq{\l_label_tl}{radicals}{\markboth{#1\ {\small(#3)}}{#1\ {\small(#3)}}}{}
  \StrLen{#1}[\hlen]
  \begin{lightbox}
   \ifnumcomp{\hlen}{>}{10}
    {%
