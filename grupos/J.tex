%%%
%%% J
%%%
\section*{J}
\addcontentsline{toc}{section}{J}

\begin{verbete}[2]{几}{ji1}
  \significado{adv.}{quase}
  \veja{几}{ji3}
\end{verbete}

\begin{verbete}[6;6]{机场}{ji1chang3}
  \significado[家,处]{s.}{aeroporto; aeródromo}
\end{verbete}

\begin{verbete}[6;11]{机票}{ji1piao4}
  \significado[张]{v.}{bilhete de avião}
  \veja{飞机票}{fei1ji1piao4}
\end{verbete}

\begin{verbete}[6;16]{机器}{ji1qi4}
  \significado[台,部,个]{s.}{máquina}
\end{verbete}

\begin{verbete}[7]{鸡}{ji1}
  \significado[只]{s.}{galo, galinha; gíria: prostituta}
\end{verbete}

\begin{verbete}[7;11]{鸡蛋}{ji1dan4}
  \significado[个,打]{s.}{ovo de galinha}
\end{verbete}

\begin{verbete}[3;10]{及格}{ji2ge2}
  \significado{v.}{atender a um padrão mínimo; passar em um exame ou teste}
\end{verbete}

\begin{verbete}[7;2]{……极了}{...ji2le5}
  \significado{expr.}{muito; extremamente}
\end{verbete}

\begin{verbete}[2]{几}{ji3}
  \significado{interr.}{quantos?, (até 10 itens); alguns?}
  \veja{几}{ji1}
\end{verbete}

\begin{verbete}[9]{给}{ji3}
  \significado{v.}{fornecer}
  \veja{给}{gei3}
\end{verbete}

\begin{verbete}[5;11]{记得}{ji4de5}
  \significado{v.}{lembrar; lembrar-se}
\end{verbete}

\begin{verbete}[5;8]{记性}{ji4xing5}
  \significado{s.}{memória (habilidade em reter informações)}
\end{verbete}

\begin{verbete}[5;7]{记住}{ji4-zhu4}
  \significado{v.}{decorar; memorizar; ter em mente}
\end{verbete}

\begin{verbete}[8;5]{季节}{ji4jie2}
  \significado[个]{s.}{estação (clima)}
\end{verbete}

\begin{verbete}[9;10]{技俩}{ji4liang3}
  \significado{s.}{truque; estratagema; ardil; esquema; estratégia; tática}
\end{verbete}

\begin{verbete}[11]{寄}{ji4}
  \significado{v.}{enviar; mandar}
\end{verbete}

\begin{verbete}[11;6]{寄存}{ji4cun2}
  \significado{v.}{depositar; deixar algo com alguém; armazenar}
\end{verbete}

\begin{verbete}[11;10]{寄递}{ji4di4}
  \significado{s.}{entrega de correspondência}
\end{verbete}

\begin{verbete}[11;8]{寄放}{ji4fang4}
  \significado{v.}{deixar algo com alguém}
\end{verbete}

\begin{verbete}[11;8]{寄居}{ji4ju1}
  \significado{s.}{morar longe de casa}
\end{verbete}

\begin{verbete}[11;8]{寄卖}{ji4mai4}
  \significado{v.}{consignar para venda}
\end{verbete}

\begin{verbete}[11;5]{寄生}{ji4sheng1}
  \significado{s.}{parasita; parasitismo}
  \significado{v.}{viver tirando vantagem dos outros; viver dentro ou sobre outro organismo como um parasita}
\end{verbete}

\begin{verbete}[11;5;5;9]{寄生生活}[\\]{ji4sheng1sheng1huo2}
  \significado{s.}{parasitismo; vida parasitária}
\end{verbete}

\begin{verbete}[11;11]{寄售}{ji4shou4}
  \significado{v.}{venda em consignação}
\end{verbete}

\begin{verbete}[11;9]{寄送}{ji4song4}
  \significado{v.}{enviar; transmitir}
\end{verbete}

\begin{verbete}[11;11]{寄宿}{ji4su4}
  \significado{s.}{embarque}
  \significado{v.}{embarcar}
\end{verbete}

\begin{verbete}[11;6]{寄托}{ji4tuo1}
  \significado{v.}{investir (sua esperança, energia, etc.) em algo; confiar (a alguém); colocar (a esperança, a energia, etc.) em}
\end{verbete}

\begin{verbete}[11;11]{寄望}{ji4wang4}
  \significado{v.}{depositar esperanças em}
\end{verbete}

\begin{verbete}[11;9]{寄养}{ji4yang3}
  \significado{v.}{embarcar; promover; colocar sob os cuidados de alguém (uma criança, animal de estimação, etc.)}
\end{verbete}

\begin{verbete}[11;4]{寄予}{ji4yu3}
  \significado{v.}{expressar; colocar (esperança, importância, etc.) em; mostrar}
\end{verbete}

\begin{verbete*}[5;10;3]{加拿大}{jia1na2da4}
  \significado{s.}{Canadá}
\end{verbete*}

\begin{verbete}[5;10;3;2]{加拿大人}[\\]{jia1na2da4·ren2}
  \significado{s.}{canadense; pessoa nascida no Canadá}
\end{verbete}

\begin{verbete}[10]{家}{jia1}
  \significado{p.c.}{para famílias ou empresas}
  \significado[个]{s.}{família; casa; sufixo de substantivos para designar um especialista em alguma atividade}
\end{verbete}

\begin{verbete}[10;8]{家具}{jia1ju4}
  \significado[件,套]{s.}{móveis; mobiliário}
\end{verbete}

\begin{verbete}[10;7]{家里}{jia1li5}
  \significado{p.d.l.}{em casa}
\end{verbete}

\begin{verbete}[10;3]{家乡}{jia1xiang1}
  \significado[个]{s.}{terra natal}
\end{verbete}

\begin{verbete}[11;7]{假声}{jia3sheng1}
  \significado{s.}{falsete}
\end{verbete}

\begin{verbete}[11;7;6]{假证件}{jia3zheng4jian4}
  \significado{s.}{documentos falsos}
\end{verbete}

\begin{verbete}[8;14]{肩膀}{jian1bang3}
  \significado{s.}{ombro}
\end{verbete}

\begin{verbete}[9;9]{俭省}{jian3sheng3}
  \significado{adj.}{econômico}
\end{verbete}

\begin{verbete}[10]{捡}{jian3}
  \significado{v.}{apanhar; recolher; coletar}
\end{verbete}

\begin{verbete}[11;9]{检查}{jian3cha2}
  \significado[次]{s.}{inspeção}
  \significado{v.}{examinar; inspecionar}
\end{verbete}

\begin{verbete}[13;8]{简单}{jian3dan1}
  \significado{adj.}{simples; sem complicações}
\end{verbete}

\begin{verbete}[4]{见}{jian4}
  \significado{v.}{ver; entrevistar; encontrar alguém; parecer (ser alguma coisa)}
\end{verbete}

\begin{verbete}[4;9]{见面}{jian4mian4}
  \significado{v.}{encontrar-se com alguém}
\end{verbete}

\begin{verbete}[6]{件}{jian4}
  \significado{p.c.}{para eventos, coisas, roupas etc.}
\end{verbete}

\begin{verbete}[8;6]{建设}{jian4she4}
  \significado{s.}{construção}
  \significado{v.}{construir}
\end{verbete}

\begin{verbete}[8;6;8]{建设性}{jian4she4xing4}
  \significado{adj.}{construtivo}
\end{verbete}

\begin{verbete}[8;6;8]{建设者}{jian4she4zhe3}
  \significado{s.}{construtor}
\end{verbete}

\begin{verbete}[8;5]{建议}{jian4yi4}
  \significado[个,点]{s.}{proposta; recomendação; sugestão}
  \significado{v.}{propor; recomendar; sugerir}
\end{verbete}

\begin{verbete*}[6;6]{江西}{jiang1xi1}
  \significado{s.}{Jiangxi}
\end{verbete*}

\begin{verbete}[12]{强}{jiang4}
  \significado{adj.}{teimoso; inflexível}
  \veja{强}{qiang2}
  \veja{强}{qiang3}
\end{verbete}

\begin{verbete}[6]{交}{jiao1}
  \significado{v.}{entregar; dar}
\end{verbete}

\begin{verbete}[6;10]{交班}{jiao1ban1}
  \significado{v.}{passar para o próximo turno de trabalho}
\end{verbete}

\begin{verbete}[6;3]{交叉}{jiao1cha1}
  \significado{v.}{cruzar; sobrepor}
\end{verbete}

\begin{verbete}[6;3;3]{交叉点}{jiao1cha1dian3}
  \significado{s.}{encruzilhada; cruzamento; junção}
\end{verbete}

\begin{verbete}[6;3;3]{交叉口}{jiao1cha1kou3}
  \significado{s.}{intersecção (rodovia)}
\end{verbete}

\begin{verbete}[6;13]{交叠}{jiao1die2}
  \significado{s.}{sobreposição}
\end{verbete}

\begin{verbete}[6;9]{交给}{jiao1gei3}
  \significado{v.}{entregar algo; dar algo}
\end{verbete}

\begin{verbete}[6;13]{交媾}{jiao1gou4}
  \significado{v.}{copular; ter relações sexuais}
\end{verbete}

\begin{verbete}[6;9]{交界}{jiao1jie4}
  \significado{p.l.}{fronteira comum; limite comum; interface}
\end{verbete}

\begin{verbete}[6;10]{交通}{jiao1tong1}
  \significado{s.}{transporte; tráfego; trânsito; comunicações; conexão}
  \significado{v.}{estar conectado}
\end{verbete}

\begin{verbete}[6;9]{交响}{jiao1xiang3}
  \significado{s.}{sinfonia}
\end{verbete}

\begin{verbete}[6;7]{交运}{jiao1yun4}
  \significado{v.}{despachar (bagagem em um aeroporto, etc.); entregar para transporte}
\end{verbete}

\begin{verbete}[8;4]{郊区}{jiao1qu1}
  \significado[个]{s.}{subúrbio; distrito suburbano; arredores}
\end{verbete}

\begin{verbete}[10;8]{胶卷}{jiao1juan3}
  \significado{s.}{filme; película; rolo de filme}
\end{verbete}

\begin{verbete}[11]{教}{jiao1}
  \significado{v.}{ensinar; lecionar}
  \veja{教}{jiao4}
\end{verbete}

\begin{verbete}[6;8;10]{交杯酒}{jiao2bei1jiu3}
  \significado{s.}{copo de vinho nupcial}
\end{verbete}

\begin{verbete}[7]{角}{jiao3}
  \significado{p.c.}{1 jiao = 10 centavos}
  \significado[个]{s.}{ângulo; esquina; chifre; em forma de chifre}
\end{verbete}

\begin{verbete}[9;3]{饺子}{jiao3zi5}
  \significado[个,只]{s.}{jiaozi; bolinhos chineses; bolinho de massa}
\end{verbete}

\begin{verbete}[11]{脚}{jiao3}
  \significado{p.c.}{para chutes }
  \significado[双,只]{s.}{pé; base (de um objeto); perna (de um animal ou objeto)}
\end{verbete}

\begin{verbete}[5]{叫}{jiao4}
  \significado{v.}{chamar-se; chamar; gritar; pedir (comida em um restaurante)}
\end{verbete}

\begin{verbete}[10]{校}{jiao4}
  \significado{v.}{verificar; comparar; revisar}
  \veja{校}{xiao4}
\end{verbete}

\begin{verbete}[11]{教}{jiao4}
  \significado{s.}{religião; ensinamento }
  \significado{v.}{causar; como fazer algo; contar (explicar como fazer algo)}
  \veja{教}{jiao1}
\end{verbete}
\begin{verbete*}[11]{教}{jiao4}
  \significado{s.}{sobrenome Jiao}
\end{verbete*}

\begin{verbete}[11;6]{教导}{jiao4dao3}
  \significado{s.}{orientação; ensino}
  \significado{v.}{instruir; ensinar}
\end{verbete}

\begin{verbete}[11;8]{教官}{jiao4guan1}
  \significado{s.}{instrutor militar}
\end{verbete}

\begin{verbete}[11;8]{教练}{jiao4lian4}
	\significado[个,位,名]{s.}{instrutor; treinador (esportes)}
\end{verbete}

\begin{verbete}[11;6]{教师}{jiao4shi1}
  \significado[个]{s.}{professor; mestre}
\end{verbete}

\begin{verbete}[11;9]{教室}{jiao4shi4}
  \significado[间]{s.}{sala de aula}
\end{verbete}

\begin{verbete}[11;11]{教授}{jiao4shou4}
  \significado[个,位]{s.}{professor (universitário)}
  \significado{v.}{instruir; palestrar sobre}
\end{verbete}

\begin{verbete}[11;11]{教堂}{jiao4tang2}
  \significado{s.}{igreja; capela}
\end{verbete}

\begin{verbete}[11;8]{教学}{jiao4xue2}
  \significado[任,门,个]{s.}{ensino; educação}
\end{verbete}

\begin{verbete}[11;8;13]{教学楼}{jiao4xue2lou2}
  \significado{s.}{edifício de salas de aula}
\end{verbete}

\begin{verbete}[11;4]{教长}{jiao4zhang3}
  \significado{s.}{reitor; imã (Islã); mulá}
\end{verbete}

\begin{verbete}[11]{接}{jie1}
  \significado{v.}{ir buscar (alguém); ir ao encontro de (alguém); receber}
\end{verbete}

\begin{verbete}[11;9]{接待}{jie1dai4}
  \significado{v.}{receber (alguém); acolher; recepcionar}
\end{verbete}

\begin{verbete}[11;5;8]{接（电话）}[\\]{jie1(dian4hua4)}
  \significado{v.}{atender (o telefone)}
\end{verbete}

\begin{verbete}[12]{街}{jie1}
  \significado[条]{s.}{rua}
\end{verbete}

\begin{verbete}[5;4]{节日}{jie2ri4}
  \significado[个]{s.}{festival; feriado}
\end{verbete}

\begin{verbete}[9;8]{结果}{jie2guo3}
  \significado{s.}{como resultado; conclusão; resultado}
  \significado{v.}{despachar; matar}
\end{verbete}

\begin{verbete}[9;11;5;8]{结婚礼服}[\\]{jie2hun1·li3·fu2}
  \significado{s.}{vestido de casamento}
\end{verbete}

\begin{verbete}[9;7]{结束}{jie2shu4}
  \significado{v.}{terminar; acabar; concluir}
\end{verbete}

\begin{verbete}[9;7;16;6]{结束辩论}[\\]{jie2shu4·bian4·lun4}
  \significado{s.}{debate de encerramento}
\end{verbete}

\begin{verbete}[9;7;3;7]{结束工作}[\\]{jie2shu4gong1zuo4}
  \significado{s.}{trabalho final}
  \significado{v.}{terminar o trabalho}
\end{verbete}

\begin{verbete}[9;7;8]{结束剂}{jie2shu4·ji4}
  \significado{s.}{finalizador}
\end{verbete}

\begin{verbete}[9;7;8]{结束区}{jie2shu4·qu1}
  \significado{s.}{zona final}
\end{verbete}

\begin{verbete}[9;7;4;5]{结束文本}[\\]{jie2shu4·wen2ben3}
  \significado{s.}{texto final}
\end{verbete}

\begin{verbete}[9;7;9]{结束语}{jie2shu4yu3}
  \significado{s.}{conclusões finais}
\end{verbete}

\begin{verbete}[8;4]{姐夫}{jie3fu5}
  \significado{s.}{marido da irmã mais velha}
\end{verbete}

\begin{verbete}[8;8]{姐姐}{jie3jie5}
  \significado[个]{s.}{irmã mais velha}
\end{verbete}

\begin{verbete}[4;8]{介绍}{jie4shao4}
  \significado{s.}{introdução; apresentação}
  \significado{v.}{fazer uma apresentação; apresentar (alguém para alguém); apresentar (alguém para um emprego, etc.)}
\end{verbete}

\begin{verbete}[7;5]{芥兰}{jie4lan2}
  \significado{s.}{couve}
\end{verbete}

\begin{verbete}[10]{借}{jie4}
  \significado{adv.}{por meio de}
  \significado{v.}{pedir emprestado; emprestar; aproveitar (uma oportunidade)}
\end{verbete}

\begin{verbete}[10;4;7]{借书证}{jie4shu1zheng4}
  \significado{s.}{cartão de biblioteca; literalmente: cartão para pedir emprestado livros}
\end{verbete}

\begin{verbete}[4;6]{今年}{jin1nian2}
  \significado{p.t.}{este ano}
\end{verbete}

\begin{verbete}[4;4]{今天}{jin1tian1}
  \significado{p.t.}{hoje}
\end{verbete}

\begin{verbete}[8;16]{金融}{jin1rong2}
  \significado{s.}{finança}
\end{verbete}

\begin{verbete}[7]{近}{jin4}
  \significado{adj.}{perto; próximo}
\end{verbete}

\begin{verbete}[7]{进}{jin4}
  \significado{p.c.}{para seções em um edifício ou complexo residencial}
  \significado{s.}{matemática: base de um sistema numérico}
  \significado{v.d.}{entrar}
\end{verbete}

\begin{verbete}[7;5;3]{进出口}{jin4chu1kou3}
  \significado{s.}{importação e exportação}
  \significado{v.}{importar e exportar}
\end{verbete}

\begin{verbete}[7;3]{进口}{jin4kou3}
  \significado{adj.}{importado}
  \significado{s.}{importação; entrada; entrada (para entrada de ar, água, etc.)}
  \significado{v.}{importar}
\end{verbete}

\begin{verbete}[7;7]{进来}{jin4lai2}
  \significado{v.d.}{entrar (para a minha localização)}
\end{verbete}

\begin{verbete}[7;5]{进去}{jin4qu4}
  \significado{v.d.}{entrar (a partir da minha localização)}
\end{verbete}

\begin{verbete*}[8]{京}{jing1}
  \significado{s.}{sobrenome Jing}
\end{verbete*}

\begin{verbete*}[8;10]{京剧}{jing1ju4}
  \significado[场]{s.}{Ópera de Beijing (Pequim)}
\end{verbete*}

\begin{verbete}[8;11]{经常}{jing1chang2}
  \significado{adv.}{constantemente; diariamente; dia-a-dia; todo dia; frequentemente; sempre; regularmente}
\end{verbete}

\begin{verbete}[8;9]{经济}{jing1ji4}
  \significado{s.}{economia}
\end{verbete}

\begin{verbete}[8;11]{经理}{jing1li3}
  \significado[个,位,名]{s.}{diretor; gerente}
\end{verbete}

\begin{verbete}[14;11]{精彩}{jing1cai3}
  \significado{adj.}{espetacular; maravilhoso}
\end{verbete}

\begin{verbete}[12;6]{景色}{jing3se4}
  \significado{s.}{paisagem; panorama}
\end{verbete}

\begin{verbete}[19;14]{警察}{jing3cha2}
  \significado[个]{s.}{polícia; oficial de polícia}
\end{verbete}

\begin{verbete}[2]{九}{jiu3}
  \significado{num.}{9, nove}
\end{verbete}

\begin{verbete}[9;11]{韭菜}{jiu3cai4}
  \significado{s.}{cebolinha chinesa; figurativo: investidores de varejo que perdem seu dinheiro para operadores mais experientes (ou seja, são ``colhidos'' como cebolinhas)}
\end{verbete}

\begin{verbete}[10]{酒}{jiu3}
  \significado[杯,瓶,罐,桶,缸]{s.}{bebida alcoólica; vinho (especialmente vinho de arroz); aguardente; licor; espíritos}
\end{verbete}

\begin{verbete}[10;11]{酒馆}{jiu3guan3}
  \significado{s.}{bar; taverna; adega}
\end{verbete}

\begin{verbete}[10;9]{酒鬼}{jiu3gui3}
  \significado{adj.}{embriagado; ébrio}
  \significado{s.}{bêbado; alcoólatra; borracho}
\end{verbete}

\begin{verbete}[5]{旧}{jiu4}
  \significado{adj.}{velho; antigo; desgastado (com a idade)}
\end{verbete}

\begin{verbete}[12]{就}{jiu4}
  \significado{adv.}{exatamente; justamente}
  \significado{v.}{realizar; se envolver em; acompanhar (em alimentos); aproveitar; avançar; empreender}
\end{verbete}

\begin{verbete}[16;3;5]{橘子汁}{ju2zi5zhi1}
  \significado[瓶,杯,罐,盒]{s.}{suco de laranja}
  \veja{橙汁}{cheng2zhi1}
  \veja{柳橙汁}{liu3cheng2zhi1}
\end{verbete}

\begin{verbete}[9;6]{举行}{ju3xing2}
  \significado{v.}{realizar (uma reunião, cerimônia, etc.); ter lugar}
\end{verbete}

\begin{verbete}[5]{句}{ju4}
  \significado{p.c.}{para orações, frases ou linhas de versos}
  \significado{s.}{sentença; cláusula; frase}
\end{verbete}

\begin{verbete}[5;3]{句子}{ju4zi5}
  \significado[个]{n}{sentença; frase; oração}
\end{verbete}

\begin{verbete}[7]{足}{ju4}
  \significado{adj.}{excessivo}
  \veja{足}{zu2}
\end{verbete}

\begin{verbete}[9;11]{觉得}{jue2de2}
  \significado{v.}{achar; sentir; pensar}
\end{verbete}

\begin{verbete}[9;11]{觉得}{jue2de5}
  \significado{v.}{sentir; pensar}
\end{verbete}


%%%%% EOF %%%%%
