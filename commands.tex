%
% Funções que uso no dicionario.tex
%

\NewDocumentCommand{\mylist}{>{\SplitList{;}}m}{
  \begin{enumerate*}[left=0pt,mode=unboxed,itemjoin={{; }}]
  \ProcessList{#1}{\insertitem}
  \end{enumerate*}
}
\NewDocumentCommand{\insertitem}{m}{\item #1}

\NewDocumentEnvironment{verbete}{ o m o m o }{%
  \StrSubstitute{#4}{r5}{r}[\result]
  \StrSubstitute{\result}{t5}{t}[\pstr]
  \edef\py{\pinyin{\pstr}}
  \leavevmode%
  \markboth{#2{\small«\py»}}{#2{\small«\py»}}
  \index[pinyin]{#2@«\py» #2}
  \index[stroke]{#2@#2 «\py»}
  \index[radical]{#2@#2 «\py»}
  \begin{minipage}[t][][t]{.475\textwidth}
  \begin{FlushLeft}
  \label{\mdfivesum{#2::#4}}
  {\tiny\ding{108}\Large\textbf{#2}}%
  \IfValueTF{#3}{%
    \IfValueT{#1}{\hfill\textsuperscript{\tiny(#1画)}}%
    \IfValueTF{#5}{#3«\py»#5}{#3«\py»}
  }{%
    \IfValueTF{#5}%
      {\IfValueT{#1}{\hfill\textsuperscript{\tiny(#1画)}} «\py»#5 }%
      {\hspace{1ex}«\py»\IfValueT{#1}{\hfill\textsuperscript{\tiny(#1画)}}}
  }
  \newline
}{%
  \end{FlushLeft}
  \end{minipage}\par
}

\NewDocumentEnvironment{verbete*}{ o m o m o }{%
  \StrSubstitute{#4}{r5}{r}[\result]
  \StrSubstitute{\result}{t5}{t}[\pstr]
  \edef\py{\pinyin{\pstr}}
  \leavevmode%
  \markboth{#2{\small«\py»}}{#2{\small«\py»}}
  \index[pinyin]{#2@«\py» \textcolor{BrickRed}{#2}}
  \index[stroke]{#2@\textcolor{BrickRed}{#2} «\py»}
  \index[radical]{#2@\textcolor{BrickRed}{#2} «\py»}
  \begin{minipage}[t][][t]{.475\textwidth}
  \begin{FlushLeft}
  \label{\mdfivesum{*::#2::#4}}
  {\tiny\ding{108}\Large \textbf{#2}}%
  \IfValueTF{#3}{%
    \IfValueT{#1}{\hfill\textsuperscript{\tiny(#1画)}}%
    \IfValueTF{#5}{#3«\py»#5}{#3«\py»}
  }{%
    \IfValueTF{#5}%
      {\IfValueT{#1}{\hfill\textsuperscript{\tiny(#1画)}} «\py»#5}%
      {\hspace{1ex}«\py»\IfValueT{#1}{\hfill\textsuperscript{\tiny(#1画)}}}
  }
  \newline
  $\hookrightarrow$\ (\textit{Substantivo Próprio})
  \newline
}{%
  \end{FlushLeft}
  \end{minipage}\par
}

\NewDocumentCommand{\significado}{ o m m }{%
  $\hookrightarrow$\ (\textit{#2})%
  \IfValueT{#1}{ {\small[p.c.: #1]}\\}%
  {\mylist{#3}}%
  \newline
}

\NewDocumentCommand{\veja}{ s m m o }{%
  \StrSubstitute{#3}{r5}{r}[\result]
  \StrSubstitute{\result}{t5}{t}[\pstr]
  \edef\py{\pinyin{\pstr}}
  \IfBooleanTF{#1}{%
    \IfValueTF{#4}{%
      \textit{Veja:} #2{\footnotesize(pág.\pageref{\mdfivesum{#4::#2::#3}})}\\%
    }{%
      \textit{Veja:} #2{\footnotesize(pág.\pageref{\mdfivesum{#2::#3}})}\\%
    }%
    \hspace{6ex}{\small«\py»}%
  }{%
    \IfValueTF{#4}{%
      \textit{Veja:} #2{\small«\py»} {\footnotesize(pág.\pageref{\mdfivesum{#4::#2::#3}})}
    }{%
      \textit{Veja:} #2{\small«\py»} {\footnotesize(pág.\pageref{\mdfivesum{#2::#3}})}
    }%
  }%
  \newline
}

\NewDocumentCommand{\variante}{ m m o }{%
  \StrSubstitute{#2}{r5}{r}[\result]
  \StrSubstitute{\result}{t5}{t}[\pstr]
  \edef\py{\pinyin{\pstr}}
  \IfValueTF{#3}{%
    $\hookrightarrow$\ variante de #1{\small«\py»} {\footnotesize(pág.\pageref{\mdfivesum{#3::#1::#2}})}
  }{%
    $\hookrightarrow$\ variante de #1{\small«\py»} {\footnotesize(pág.\pageref{\mdfivesum{#1::#2}})}
  }%
  \newline
}

\DeclareRobustCommand{\&}{%
    \ifdim\fontdimen1\font>0pt
        \textsl{\symbol{`\&}}%
    \else
        \symbol{`\&}%
    \fi
}

